...
const analyzeLimiter = rateLimit({
  ...
  message: { error: 'Analyze limit reached. Please wait before submitting another report.' },
});

// ── Memory-only multer (HIPAA: never write files to disk) ────────────────────
const upload = multer({
...
// ── POST /api/analyze ─────────────────────────────────────────────────────────
router.post('/', requireAuth, analyzeLimiter, upload.single('file'), async (req: Request, res: Response) => {
    ...
    if (req.file) {
      ...
      if (fileMime === 'application/pdf') {
        ...
        buffer = await sharp(buffer)
          ...
          .jpeg({ quality: 88 })
          .withMetadata() // strip EXIF — HIPAA compliance (empty = remove all)
          .toBuffer();
        imageBase64 = buffer.toString('base64');
        mimeType = 'image/jpeg';
      }
...
router.post(
  ...
  requireAuth,
  [
    body('reportA').isString().isLength({ min: 10, max: 8000 }),
    body('reportB').isString().isLength({ min: 10, max: 8000 }),
  ],
  async (req: Request, res: Response) => {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
